...
// AudioPP ///////////////////////////////////////////////////////////////////
msdk_ts_BLOCK(b_MFXAudioPP_Query){
    mfxStatus&      mfxRes      = var_def<mfxStatus>     ("mfxRes",      MFX_ERR_NONE);
    mfxStatus&      expectedRes = var_def<mfxStatus>     ("expectedRes", MFX_ERR_NONE);
    mfxSession&     session     = var_def<mfxSession>    ("session",     0);
    mfxAudioParam*& pIn         = var_def<mfxAudioParam*>("p_mfxap_in",  NULL);
    ...
    TRACE_FUNC3(MFXAudioPP_Query, session, pIn, pOut);
    mfxRes = MFXAudioPP_Query(session, pIn, pOut);
    TRACE_PAR(mfxRes);
    TRACE_PAR(pOut);
    CHECK_STS(mfxRes, expectedRes);
...
msdk_ts_BLOCK(b_MFXAudioPP_QueryIOSSize){
    mfxAudioAllocRequest* d = NULL;
    mfxStatus&             mfxRes      = var_def<mfxStatus>             ("mfxRes",      MFX_ERR_NONE);
    mfxStatus&             expectedRes = var_def<mfxStatus>             ("expectedRes", MFX_ERR_NONE);
    mfxSession&            session     = var_def<mfxSession>            ("session",     0);
    mfxAudioParam*&        pIn         = var_def<mfxAudioParam*>        ("p_mfxap_in",  NULL);
    if(!is_defined("vpp_request")) 
        d = (mfxAudioAllocRequest*)alloc(2*sizeof(mfxAudioAllocRequest));
    mfxAudioAllocRequest*& pRequest    = var_def<mfxAudioAllocRequest*> ("vpp_request",   d);

    TRACE_FUNC3(MFXAudioPP_QueryIOSSize, session, pIn, pRequest);
    mfxRes = MFXAudioPP_QueryIOSSize(session, pIn, pRequest);
    TRACE_PAR(mfxRes);
    if(pRequest){
        TRACE_PAR(pRequest[0]);
        TRACE_PAR(pRequest[1]);
...
msdk_ts_BLOCK(b_MFXAudioPP_Init){
    mfxStatus&      mfxRes      = var_def<mfxStatus>    ("mfxRes",      MFX_ERR_NONE);
    mfxStatus&      expectedRes = var_def<mfxStatus>    ("expectedRes", MFX_ERR_NONE);
    mfxSession&     session     = var_def<mfxSession>   ("session",     0);
    mfxAudioParam*& pIn         = var_def<mfxAudioParam*>("p_mfxap_in", NULL);

    TRACE_FUNC2(MFXAudioPP_Init, session, pIn);
    mfxRes = MFXAudioPP_Init(session, pIn);
    TRACE_PAR(mfxRes);
    CHECK_STS(mfxRes, expectedRes);
...
msdk_ts_BLOCK(b_MFXAudioPP_Reset){
    mfxStatus&      mfxRes      = var_def<mfxStatus>    ("mfxRes",      MFX_ERR_NONE);
    mfxStatus&      expectedRes = var_def<mfxStatus>    ("expectedRes", MFX_ERR_NONE);
    mfxSession&     session     = var_def<mfxSession>   ("session",     0);
    mfxAudioParam*& pIn         = var_def<mfxAudioParam*>("p_mfxap_in", NULL);

    TRACE_FUNC2(MFXAudioPP_Reset, session, pIn);
    mfxRes = MFXAudioPP_Reset(session, pIn);
    TRACE_PAR(mfxRes);
    CHECK_STS(mfxRes, expectedRes);
...
msdk_ts_BLOCK(b_MFXAudioPP_Close){
    mfxStatus&      mfxRes      = var_def<mfxStatus>    ("mfxRes",      MFX_ERR_NONE);
    mfxStatus&      expectedRes = var_def<mfxStatus>    ("expectedRes", MFX_ERR_NONE);
    mfxSession&     session     = var_def<mfxSession>   ("session",     0);

    TRACE_FUNC1(MFXAudioPP_Close, session);
    mfxRes = MFXAudioPP_Close(session);
    TRACE_PAR(mfxRes);
    CHECK_STS(mfxRes, expectedRes);
...
msdk_ts_BLOCK(b_MFXAudioPP_GetAudioParam){
    mfxStatus&      mfxRes      = var_def<mfxStatus>    ("mfxRes",      MFX_ERR_NONE);
    mfxStatus&      expectedRes = var_def<mfxStatus>    ("expectedRes", MFX_ERR_NONE);
    mfxSession&     session     = var_def<mfxSession>   ("session",     0);
    mfxAudioParam*& pIn         = var_def<mfxAudioParam*>("p_mfxap_in", NULL);

    TRACE_FUNC2(MFXAudioPP_GetAudioParam, session, pIn);
    mfxRes = MFXAudioPP_GetAudioParam(session, pIn);
    TRACE_PAR(mfxRes);
    TRACE_PAR(pIn);
    CHECK_STS(mfxRes, expectedRes);
...
msdk_ts_BLOCK(b_MFXAudioPP_RunFrameAudioPPAsync){
    mfxStatus&      mfxRes     = var_def<mfxStatus>         ("mfxRes",     MFX_ERR_NONE);
    mfxSession&     session    = var_def<mfxSession>        ("session",    0);
    mfxBitstream*&  pBsIn      = var_def<mfxBitstream*>     ("p_bs_in",    NULL);
    mfxBitstream*&  pBsOut     = var_def<mfxBitstream*>     ("p_bs_out",   NULL);
    mfxU16&         numIn      = var_def<mfxU16>            ("numin",      0);
    mfxU16*&        pNumIn     = var_def<mfxU16*>           ("p_numin",    &numIn);
    mfxU16&         numOut     = var_def<mfxU16>            ("numout",     0);
    mfxU16*&        pNumOut    = var_def<mfxU16*>           ("p_numout",   &numOut);
    mfxSyncPoint&   syncp      = var_def<mfxSyncPoint>      ("syncp",      NULL);
    mfxSyncPoint*&  pSyncp     = var_def<mfxSyncPoint*>     ("p_syncp",    &syncp);

    TRACE_FUNC5(MFXAudioPP_RunFrameAudioPPAsync, session, pBsIn, pBsOut, numOut, pSyncp);
    mfxRes = MFXAudioPP_RunFrameAudioPPAsync(session, &pBsIn, numIn, &pBsOut, numOut, pSyncp);
    TRACE_PAR(mfxRes);
    //TRACE_PAR(pBsIn);
    TRACE_PAR(pBsOut);
    TRACE_PAR(pNumOut);
