...
If the `EncodedOrder` field in the [mfxInfoMFX](#mfxInfoMFX) structure is true, input frames enter the encoder in the order of their encoding. The `FrameOrder` field in the [mfxFrameData](#mfxFrameData) structure of each frame can be assigned with `MFX_FRAMEORDER_UNKNOWN` and SDK will fill this value internally or assigned with user-defined frame identificator. If `EncodedOrder` is false, `FrameOrder` field doesn't affect encoding operation at all. In both cases value generated by SDK or provided by user will be returned in [mfxExtAvcEncodedFrameInfo](#mfxExtAvcEncodedFrameInfo) structure attached to bitstream.
...
The `mfxExtAVCEncodedFrameInfo` structure is also used by the HEVC encoder as `mfxExtHEVCEncodedFrameInfo`.
...
`FrameOrder` | Frame order of encoded picture. Value is set by SDK internally if FrameOrder value in input frame's [mfxFrameData](#mfxFrameData) is not provided (equal to `MFX_FRAMEORDER_UNKNOWN`). Otherwise it is assigned with a value provided by user.
...
<a id='FrameOrder'>`FrameOrder`</a> | Current frame counter for the top field of the current frame.
...
`DeltaQP` | For block i: QP[i] = BrcQP[i] + DeltaQP[i]. Valid when Mode = MFX_MBQP_MODE_QP_DELTA .
...
If this option is turned ON (Granularity != MFX_PARTIAL_BITSTREAM_NONE), then the encoder can output bitstream by part based with required granularity. <br>
