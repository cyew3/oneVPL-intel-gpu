...
std::string dump_mfxFrameInfo(const std::string structName, mfxFrameInfo *info)
{
    ...
    if(info){
      str += structName + ".reserved=" + ToString(info->reserved[0]) + ", " + ToString(info->reserved[1]) + ", " +
...
std::string dump_mfxFrameAllocRequest(const std::string structName, mfxFrameAllocRequest *frameAllocRequest)
{
    ...
    if(frameAllocRequest){
      str += structName + ".reserved=" + ToString(frameAllocRequest->reserved[0]) + ", " + ToString(frameAllocRequest->reserved[1]) + ", " +
...
std::string dump_mfxInfoVPP(const std::string structName, mfxInfoVPP *vpp)
{
    ...
    if(vpp){
      str += structName + ".reserved=" + ToString(vpp->reserved[0]) + ", " + ToString(vpp->reserved[1]) + ", " +
...
std::string dump_mfxInfoMFX(const std::string structName, mfxInfoMFX *mfx)
{
    ...
    if(mfx){
      str += structName + ".reserved=" + ToString(mfx->reserved[0]) + ", " + ToString(mfx->reserved[1]) + ", " + ToString(mfx->reserved[2]) + ", "
...
std::string dump_mfxVideoParam(const std::string structName, mfxVideoParam *videoParam)
{
    ...
    if(videoParam){
      str += structName + ".reserved=" + ToString(videoParam->reserved[0]) + ", " + ToString(videoParam->reserved[1]) + ", " + ToString(videoParam->reserved[2]) + "\n" +
...
std::string dump_mfxFrameData(const std::string structName, mfxFrameData *frameData)
{
    ...
    if(frameData){
      str += structName + ".reserved=" + ToString(frameData->reserved[0]) + ", " + ToString(frameData->reserved[1]) + ", " + ToString(frameData->reserved[2]) + ", "
      ...
      if(Log::GetLogLevel() == LOG_LEVEL_FULL){
        ...
        structName + ".R=" + ToHexFormatString(frameData->R) + "\n" +
        structName + ".UV=" + ToHexFormatString(frameData->UV) + "\n" +
        structName + ".VU=" + ToHexFormatString(frameData->VU) + "\n" +
        structName + ".CbCr=" + ToHexFormatString(frameData->CbCr) + "\n" +
        structName + ".CrCb=" + ToHexFormatString(frameData->CrCb) + "\n" +
        structName + ".Cb=" + ToHexFormatString(frameData->Cb) + "\n" +
        structName + ".U=" + ToHexFormatString(frameData->U) + "\n" +
        structName + ".G=" + ToHexFormatString(frameData->G) + "\n" +
        structName + ".Cr=" + ToHexFormatString(frameData->Cr) + "\n" +
        structName + ".V=" + ToHexFormatString(frameData->V) + "\n" +
        structName + ".B=" + ToHexFormatString(frameData->B) + "\n" +
        structName + ".A=" + ToHexFormatString(frameData->A) + "\n";
      }
...
std::string dump_mfxDecodeStat(const std::string structName, mfxDecodeStat *decodeStat)
{
    ...
    if(decodeStat){
      str += structName + ".reserved=" + ToString(decodeStat->reserved[0]) + ", " + ToString(decodeStat->reserved[1]) + ", " + ToString(decodeStat->reserved[2]) + ", "
...
std::string dump_mfxPayload(const std::string structName, mfxPayload *payload)
{
    ...
    if(payload){
      str += structName + ".reserved=" + ToString(payload->reserved[0]) + ", " + ToString(payload->reserved[1]) + ", "
...
std::string dump_mfxVPPStat(const std::string structName, mfxVPPStat *vppStat)
{
    ...
    if(vppStat){
      str += structName + ".reserved=" + ToString(vppStat->reserved[0]) + ", " + ToString(vppStat->reserved[1]) + ", " + ToString(vppStat->reserved[2]) + ", "
