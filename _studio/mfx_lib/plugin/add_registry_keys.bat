...
set  VPP_SW_X86_PLUGIN_NAME=mfxplugin32_vpp_sw.dll
...
set MSDK_PLUGIN_DISPATCH_REGISTRY=HKEY_LOCAL_MACHINE\SOFTWARE\Intel\MediaSDK\Plugin
set MSDK_PLUGIN_DISPATCH_REGISTRY_WOW3264=HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Intel\MediaSDK\Plugin
...
reg add "%MSDK_PLUGIN_DISPATCH_REGISTRY%\%VPP_SW_PLUGIN_GUID%" /v CodecID /t REG_DWORD /d 0 /f
...
if "%PROCESSOR_ARCHITECTURE%"=="x86" (
:: for x86 systems :: 
    reg add "%MSDK_PLUGIN_DISPATCH_REGISTRY%\%VPP_SW_PLUGIN_GUID%" /v Path /t REG_SZ /d "%PATH_TO_BRANCH%build\\win_Win32\\bin\\%VPP_SW_PLUGIN_GUID%\\%VPP_SW_X86_PLUGIN_NAME%" /f
) else (
:: for x64 systems
...
if NOT "%PROCESSOR_ARCHITECTURE%"=="x86" (
    ...
    reg add "%MSDK_PLUGIN_DISPATCH_REGISTRY_WOW3264%\%VPP_SW_PLUGIN_GUID%" /v CodecID /t REG_DWORD /d 0 /f
    ...
    reg add "%MSDK_PLUGIN_DISPATCH_REGISTRY_WOW3264%\%VPP_SW_PLUGIN_GUID%" /v Path /t REG_SZ /d "%PATH_TO_BRANCH%build\\win_Win32\\bin\\%VPP_SW_PLUGIN_GUID%\\%VPP_SW_X86_PLUGIN_NAME%" /f
